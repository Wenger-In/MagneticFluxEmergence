...
class Basic(object):
    ...
    def generate_filename(self, suffix): # edited by RuiZhuo, 20240725
        date_time_str = self.recordset.split('[')[1].split(']')[0]
        date_time_format = '%Y.%m.%d_%H:%M:%S'
        file_date_time = dt_obj.strptime(date_time_str, date_time_format)
        filename_date_time = file_date_time.strftime('%Y%m%d%H%M%S')
        filename = f"{filename_date_time}_{suffix}.fits"
        return filename
    ...
    def get_data(self):
        ...
        save_dir = 'E:/Research/Data/HMI/VectorField/fits/'
        
        filenames = {
            'azimuth': self.generate_filename('azimuth'),
            'field': self.generate_filename('field'),
            'inclination': self.generate_filename('inclination'),
            'disambig': self.generate_filename('disambig')
        }

        def download_file(segment, filename): # edited by RuiZhuo, 20240725
            filepath = os.path.join(save_dir, filename)
            if not os.path.exists(filepath):
                url = f"{baseurl}{segment}"
        ...
        azimuth = download_file(segments.azimuth[0], filenames['azimuth'])
        field = download_file(segments.field[0], filenames['field'])
        inclination = download_file(segments.inclination[0], filenames['inclination'])
        disambig = download_file(segments.disambig[0], filenames['disambig'])
        
        for key, path in filenames.items():
            print(f"{key.capitalize()} file saved to: {os.path.join(save_dir, path)}")
