...
if((initfile = fopen("allboundaries.dat","r")) == NULL) { printf("\n Error allboundaries.dat "); exit(1);}
for(iy=0;iy<ny;iy++)
for(ix=0;ix<nx;ix++)
{
i=ny*ix+iy;
...
{power=power*2;
/* printf("\n %i, %i",ip,power); */
if (nx<=power){nx2=power;break;}}
power=1;
for(ip=1;ip<16;ip++)
{power=power*2;
/* printf("\n %i, %i",ip,power); */
if (ny<=power){ny2=power;break;}}
printf("\n\n nx2= %i, \t ny2= %i",nx2,ny2);
xoff=(nx2-nx)/2;
yoff=(ny2-ny)/2;
printf("\n X-offset= %i, \t Y-offset= %i",xoff,yoff);
nxny2=nx2*ny2;
if((initfile = fopen("grid2n2.ini","w")) == NULL) { printf("\n Error grid.ini"); exit(1);}
fprintf(initfile,"%s \n\t %i","nx",nx2);
fprintf(initfile,"\n%s \n\t %i","ny",ny2);
fprintf(initfile,"\n%s \n\t %i","nz",nz);
fprintf(initfile,"\n%s \n\t %i","xoffset",xoff);
...
if((streamw = fopen("zboundaries2n2.dat","w")) == NULL) { printf("\n Error zboundaries2n2.dat "); exit(1);}
for(iy=0;iy<ny2;iy++)
for(ix=0;ix<nx2;ix++)
{
i2=ny2*ix+iy;
...
if(isign == -1)
{for(k=1;k<n+1;k++){data[k]=data[k]/nn;}}
...
if((initfile = fopen("grid2n2.ini","r")) == NULL) { printf("\n Error grid2n2.ini"); exit(1);}
fscanf(initfile,"%s %i",&leer,&nx);
fscanf(initfile,"%s %i",&leer,&ny);
fscanf(initfile,"%s %i",&leer,&nz);
...
Bzorig =(double *) calloc(2*nxny, sizeof(double)); /* Define as complex Field, maybe change later  */
if((initfile = fopen("zboundaries2n2.dat","r")) == NULL) { printf("\n Error zboundaries2n2.dat "); exit(1);}
for(iy=0;iy<ny;iy++)
for(ix=0;ix<nx;ix++)
{
i=ny*ix+iy;
...
for(ix=0;ix<nx;ix++) /* specify kx,ky,kz2, k */
 { if(ix<=nx/2){ix2=ix;}
    ...
    i2=2*i; /* for complex fields */
    if(ix==0 && iy==0){
    iphihat0[i2]=0.0; iphihat0[i2+1]=0.0;}
    else
    {
...
if(1==2){
if((initfile = fopen("B02n2.bin","wb")) == NULL) { printf("\n Error B02n2.bin"); exit(1);}
...
if((initfile = fopen("B0.bin","wb")) == NULL) { printf("\n Error B0.bin"); exit(1);}
