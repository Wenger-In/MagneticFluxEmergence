...
(alpha*(pi*n/Ly)*sin(pi*m*x0/Lx)*cos(pi*n*y0/Ly) 
-rmn[m][n]*(pi*m/Lx)*cos(pi*m*x0/Lx)*sin(pi*n*y0/Ly)));
}
return bx0;
}
...
(alpha*(pi*m/Lx)*cos(pi*m*x0/Lx)*sin(pi*n*y0/Ly) 
+rmn[m][n]*(pi*n/Ly)*sin(pi*m*x0/Lx)*cos(pi*n*y0/Ly)));
}
return by0;
}
...
if((streamw = fopen("allboundaries.dat","r")) == NULL) { printf("\n Error "); exit(1);}
for(iy=0;iy<ny;iy++)
for(ix=0;ix<nx;ix++)
{
i2=ny*ix+iy;
