...
printf("\n\n Ls=%lf \t mu0= %lf",Ls,mu0);
if((initfile = fopen("grid.ini","r")) == NULL) { printf("\n Error"); exit(1);}
...
if((streamw = fopen("prot.log","w")) == NULL) { printf("\n Error prot.log"); exit(1);}
fclose(streamw);
alpha=0.0;
if(calcb==1 && maxit==-1)
{
...
{
if((streamw = fopen("B0.bin","rb")) == NULL) { printf("\n Error B.bin"); exit(1);}
...
if((streamw = fopen("mask.dat","r")) == NULL) { printf("\n Error mask.dat"); exit(1);}
for(iy=0;iy<ny;iy++)
for(ix=0;ix<nx;ix++)
{
i1=ny*ix+iy;
...
for(i=0;i<nxnynz;i++)
{
Bx1[i]=Bx2[i]=Bx[i];By1[i]=By2[i]=By[i];Bz1[i]=Bz2[i]=Bz[i];
...
if(maxit==987654)
{
if((streamw = fopen("wa.bin","rb")) == NULL) { printf("\n Error wa.bin"); exit(1);}
fread(wa0,sizeof(double)*1*nxnynz,1,streamw);
printf("\n Read wa finished ");
if((streamw = fopen("wb.bin","rb")) == NULL) { printf("\n Error wb.bin"); exit(1);}
...
if((streamw = fopen("Iteration_start.mark","w")) == NULL) { printf("\n Error mark"); exit(1);}
fclose(streamw);
...
i=ix*nynz+iy*nz+iz;
i1=ix*ny+iy;
L3=L3+mask[i1]*((Bx[i]-Bxorig[i1])*(Bx[i]-Bxorig[i1])
+(By[i]-Byorig[i1])*(By[i]-Byorig[i1])+(Bz[i]-Bzorig[i1])*(Bz[i]-Bzorig[i1]));
}
/* L3=L3/nx/ny; */
/* printf("\n nave= %i",nave); */
L3=L3/nave;
...
if(it==0) {oldL=L;}
if (restore == 1) {L = oldL;}   /* restore when L is rejected */
...
if((streamw = fopen("step.log","w")) == NULL) { printf("\n Error help.log"); exit(1);}
fprintf(streamw,"%i",it/diagstep);
fclose(streamw);
if((streamw = fopen("prot.log","a")) == NULL) { printf("\n Error prot.log"); exit(1);}
...
*/
printf("\n Iteration_stop.mark \n"); 
if((streamw = fopen("Iteration_stop.mark","w")) == NULL) { printf("\n Error mark"); exit(1);}
fclose(streamw);
if(1==1) /* norm B, only inner Region */
{
#ifdef _OPENMP
...
if((streamw = fopen("Bout.bin","wb")) == NULL) { printf("\n Error Bout.bin"); exit(1);}
