...
if((streamw = fopen("input.dat","r")) == NULL) { printf("\n Error input.dat "); exit(1);}
for(iy=0;iy<ny;iy++)
for(ix=0;ix<nx;ix++)
{
i=ny*ix+iy; 
...
Emag=ihelp=0.0;
#ifdef _OPENMP
#pragma omp parallel for private (i)reduction(+:Emag) reduction(+:ihelp)
#endif
for(i=0;i<nxny;i++)
...
L1=L2=L3=L4=L5=0.0;
#ifdef _OPENMP
#pragma omp parallel for private (i,ix,iy) reduction(+:term1a) reduction(+:term1b)reduction(+:term1c1) reduction(+:term1c2) reduction(+:term2a1) reduction(+:term2a2) reduction(+:term2b1) reduction(+:term2b2) reduction(+:term2c1) reduction(+:term2c2) reduction(+:L3)
#endif
for(ix=0;ix<nx;ix++)
for(iy=0;iy<ny;iy++)
{
...
{
i=ny*ix+iy;
Bx1[i]=Bx[i]+dt*(mu1*(-2*term1a*Bz[i]+4*term1c*Bx[i])-mu3*2*(Bx[i]-Bxo[i])
-mu4*term4a[i]
+mu2*(4*term2a*x[ix]*Bx[i]+4*term2b*y[iy]*Bx[i]-2*term2c*y[iy]*Bz[i]));
By1[i]=By[i]+dt*(mu1*(-2*term1b*Bz[i]+4*term1c*By[i])-mu3*2*(By[i]-Byo[i])
-mu4*term4b[i]
+mu2*(4*term2a*x[ix]*By[i]+4*term2b*y[iy]*By[i]+2*term2c*x[ix]*Bz[i]));
...
if((streamw = fopen("output.dat","w")) == NULL) { printf("\n Error output.dat "); exit(1);}
for(iy=0;iy<ny;iy++)
for(ix=0;ix<nx;ix++)
{
i=ny*ix+iy; 
