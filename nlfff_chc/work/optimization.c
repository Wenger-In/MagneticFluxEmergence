...
zaehl=0;
#ifdef _OPENMP
#pragma omp parallel for private (i,ix,iy,iz,bx,by,bz,cby,cbz,fx,fy,fz,divB,b2) reduction(+:helpL)
#endif
for(ix=0;ix<nx;ix++)
for(iy=0;iy<ny;iy++)
for(iz=0;iz<nz;iz++)
...
#ifdef _OPENMP
#pragma omp parallel for private (i,ix,iy,iz,o2a,o2b, term1x, term2x, term3x, term4x, term5ax, term5bx, term1y, term2y, term3y, term4y, term5ay,term5by,term1z, term2z, term3z, term4z, term5az,term5bz, term6x, term6y, term6z, term7x, term7y, term7z) reduction(+:dummy1) reduction(+:dummy2) reduction(+:dummy3) reduction(+:dummy4) reduction(+:dummy5) reduction(+:dummy6)
#endif
for(ix=0;ix<nx;ix++)
for(iy=0;iy<ny;iy++)
for(iz=0;iz<nz;iz++)
...
helpL=0.0;
#ifdef _OPENMP
#pragma omp parallel for private (i,ix,iy,iz,bx,by,bz,b2) reduction(+:helpL)
#endif
