...
if((initfile = fopen("param1.ini","r")) == NULL) { printf("\n Error"); exit(1);}
...
if((initfile = fopen("Cmn1.ini","r")) == NULL) { printf("\n Error Cmn"); exit(1);} 
for(n=0; n<maxn+1; n++)
for(m=0; m<maxm+1; m++)
{
...
printf("\n dx= %lf",dx/(1.0e6));
i=(nxmax-nx)/2;
for(ix=0; ix<nx; ix++){x[ix]=(i+ix)*dx;}
for(iy=0; iy<ny; iy++){y[iy]=(i+iy)*dx;}
for(iz=0; iz<nz; iz++){z[iz]=iz*dx;}

/* Rand weg, wo B null wird */

for(ix=0; ix<nx; ix++){x[ix]=Lx*0.005+ix*0.99*Lx/(nx-1);}
for(iy=0; iy<ny; iy++){y[iy]=Ly*0.005+iy*0.99*Ly/(ny-1);}
for(iz=0; iz<nz; iz++){z[iz]=iz*0.99*Lz/(nz-1);}

for(i=0; i<nxnynz; i++){Bx[i]=By[i]=Bz[i]=0.0;}
...
*/
if((streamw = fopen("B0.bin","wb")) == NULL) { printf("\n Error B0.bin"); exit(1);}
...
{
dummy4=0.0;
if((streamw = fopen("B.dat","r")) == NULL) { printf("\n Error B.dat"); exit(1);}
for(ix=0;ix<nx;ix++)
for(iy=0;iy<ny;iy++)
for(iz=0;iz<nz;iz++)
{
