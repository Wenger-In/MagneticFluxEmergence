...
#define DUMP i=nynz*ix+nz*iy+iz;fprintf(streamw,"%lf\n", Bx[i]); fprintf(streamw,"%lf\n", By[i]); fprintf(streamw,"%lf\n", Bz[i]); 
/* Macros */
...
B = (double *) calloc(3*nxnynzall, sizeof(double));
 ...
 /* Read B from file */
if((streamw = fopen(argv[1],"rb")) == NULL) { printf("\n Read Error **.bin"); exit(1);}
...
sigi=0.0;
if(jb1>0.0){sigi=jxb/jb1;}
totj3d=totj3d+j3d;
/* totsig=totsig+sigi; */
totsig=totsig+j3d*sigi;
}
helpL=helpL*h*h*h;
helpL1=helpL1*h*h*h;
helpL2=helpL2*h*h*h;
sigj=totsig/totj3d;
angle=asin(sigj)*180.0/3.14159265;
...
if(k==0)
{
if((streamw = fopen("NLFFFquality.log","w")) == NULL) { printf("\n Error NLFFFquality.log"); exit(1);}
fprintf(streamw,"\n Quality Check of NLFFF field");
fprintf(streamw,"\n Input File: %s  \n",argv[1]);
fprintf(streamw,"\n Full Box, nx=%i, ny=%i,nz=%i, nd=%i \n",nx,ny,nz,orignd);
}
else{fprintf(streamw,"\n\n Inner Region, nx=%i, ny=%i,nz=%i \n",nx,ny,nz);}
...
free(oxb); free(oyb); free(ozb); free(DivB);
}
fclose(streamw);
