...
if((streamw = fopen(argv[1],"r")) == NULL) { printf("\n Error reading"); exit(1);}
for(iy=0;iy<ny;iy++)
for(ix=0;ix<nx;ix++)
{
i=ny*ix+iy;
...
if((streamw = fopen("minigrid.ini","w")) == NULL) { printf("\n Error writing"); exit(1);}
fprintf(streamw,"nx \n");
fprintf(streamw,"\t %i \n",nx2);
fprintf(streamw,"ny \n");
fprintf(streamw,"\t %i \n",ny2);
...
/***********************************************************************************/
if((streamw = fopen(argv[2],"w")) == NULL) { printf("\n Error writing"); exit(1);}
for(iy2=0;iy2<ny2;iy2++)
for(ix2=0;ix2<nx2;ix2++)
{
i2=ny2*ix2+iy2;
