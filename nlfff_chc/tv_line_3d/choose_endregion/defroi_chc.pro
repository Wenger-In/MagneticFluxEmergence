; $Id: //depot/Release/ENVI50_IDL82/idl/idldir/lib/defroi.pro#1 $
;
; Copyright (c) 1987-2012, Exelis Visual Information Solutions, Inc. All
...
;-
;
Function Defroi_chc, Sx, Sy, Xverts, Yverts, X0=x0, Y0=y0, ZOOM = ZOOM, $
    ...
    while (1b) do begin  ; infinite loop
        ...
        1: begin                ; Add a point.
            ...
            PLOTS, xverts[n]*zoom + x0,yverts[n]*zoom + y0, $
                /data, COLOR=nc1, /NOCLIP, PSYM=-3,THICK=3., $
                CONTINUE=(n gt 0)
            ; record the posion of these points
            n++         ; Next point           
            break
           end
    ...
    if keyword_set(restore) then begin
        ...
        plots, [xverts[0],xverts[n-1]]*zoom+x0, $
            ...
            /data,color = nc1,/noclip                 ;Complete polygon
    endif else begin
