...
x2=-(a1+c1*y2)/(b1+d1*y2)
if x1 ge 0 and x1 le 1 and y1 ge 0 and y1 le 1 then res[j,i*2]=a3+b3*x1+c3*y1+d3*x1*y1

if x2 ge 0 and x2 le 1 and y2 ge 0 and y2 le 1 then res[j,i*2+1]=a3+b3*x2+c3*y2+d3*x2*y2
endif
endif
endfor
endfor
...
r=res[0,*]
wr=where(r ne 0,nwr)
if nwr eq 0 then fr=0 else fr=-1
if nwr eq 2 then if r[wr[0]]*r[wr[1]] lt 0 then fr=1


t=res[1,*]
wt=where(t ne 0,nwt)
if nwt eq 0 then ft=0 else ft=-1
if nwt eq 2 then if t[wt[0]]*t[wt[1]] lt 0 then ft=1


p=res[2,*]
wp=where(p ne 0,nwp)
if nwp eq 0 then fp=0 else fp=-1
if nwp eq 2 then if p[wp[0]]*p[wp[1]] lt 0 then fp=1


yy=0
if (fr ne -1 and ft ne -1 and fp ne -1) and (fr eq 1 or ft eq 1 or fp eq 1) then yy=1
return, yy
end
