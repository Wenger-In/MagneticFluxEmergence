function supper_trace_flines_xyz_bot_endpoint,bx,by,bz,sx0,sy0,sz0
st=systime(1)
maxiter=3e4
...
np=n_elements(sx0)
;line={n:np,x:fltarr(np,2),y:fltarr(np,2),z:fltarr(np,3)}
line={n:np,x:ptrarr(np),y:ptrarr(np),z:ptrarr(np),l:fltarr(np)}
...
;line.x[sx[cok[j],0],* ]=reform(sx[cok[j],[1,endp-1]])
;line.y[sx[cok[j],0],* ]=reform(sy[cok[j],[1,endp-1]])
;line.z[sx[cok[j],0],* ]=[reform(sz[cok[j],[1,endp-1]]),max(sz[cok[j],1:endp-1])]
...
line.l[sx[cok[j],0] ]=endp-1
endif; else line.z[sx[cok[j],0],2]=-1
