...
class Grid:
    ...
    def field_from_array(self, array):
        """ Factory function to create a Field with suitable shape for the grid. The values
            are directly given by a user-defined array
        
        :param array: an array -> real number
        :return: a new Field filled with array values
        """
        field = Field(self)
        field.values = array
        return field
        ...
        for ix in range(nx):
            for iy in range(ny):
                for iz in range(nz):
                    ...
                    if ixf == np.shape(field.values)[0] or iyf == np.shape(field.values)[1] or izf == np.shape(field.values)[2]:
                        coarse_field.values[ix, iy, iz] = field.values[-1, -1, -1]
                    else:
